    ...
    <div className="space-y-8 animate-fadeIn py-4">
      <div className="border-b border-slate-200 pb-5">
        ...
        <p className="text-sm text-slate-500 mt-2 max-w-2xl leading-relaxed">
          Pahami persen sebagai per seratus dan hubungan pecahan dengan persen.
        </p>
      </div>
        ...
        {percentVisualizations.map((item) => (
          <div key={item.id} className="bg-white border border-slate-200 rounded-3xl overflow-hidden flex flex-col hover:shadow-xl transition-all duration-300 group">
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <Link
                to={item.href}
                className="w-full inline-flex justify-center items-center gap-2 bg-slate-100 hover:bg-amber-500 hover:text-white text-slate-700 font-bold px-4 py-2.5 rounded-xl text-sm transition-colors shadow-sm mt-auto"
              >
                <Play size={14} />
                <span>Buka</span>
              </Link>
