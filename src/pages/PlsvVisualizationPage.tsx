      ...
      >
        <div>
          <div className="flex items-center gap-1.5 mb-1.5">
            {item.icon ? <item.icon size={14} className="text-teal-500" /> : <Box size={14} className="text-teal-500" />}
            <span className="text-[10px] font-bold text-teal-600 tracking-wide uppercase">{item.materialTopic}</span>
          </div>
      ...
      {isExpanded && (
        <div className="animate-fadeIn border-t border-slate-100 flex flex-col flex-1">
          ...
          <div className="h-24 md:h-40 bg-slate-50 border-b border-slate-100 relative overflow-hidden flex items-center justify-center p-3 md:p-4 shrink-0">
            ...
            <div className="relative z-10 flex flex-col items-center gap-1 font-mono text-xs font-bold text-slate-700 bg-white/75 px-3 py-2 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex items-center gap-2">
                <span className="bg-teal-50 text-teal-800 px-1.5 py-0.5 rounded border border-teal-200">x + 3</span>
                <span className="text-slate-400">=</span>
                <span className="bg-slate-100 text-slate-800 px-1.5 py-0.5 rounded border border-slate-200">8</span>
              </div>
              <span className="text-[10px] text-slate-400">↓ (Kurangi 3 di kedua ruas)</span>
              <div className="flex items-center gap-2">
          ...
          <div className="p-6 flex-1 flex flex-col">
            ...
            <Link
              to={item.href}
              className="w-full inline-flex justify-center items-center gap-2 bg-slate-900 hover:bg-teal-600 text-white font-bold px-4 py-3 rounded-xl text-sm transition-colors shadow-md mt-auto animate-pulse"
            >
...
export default function PlsvVisualizationPage() {
  return (
    <div className="space-y-8 animate-fadeIn py-4">
      ...
      <div className="border-b border-slate-200 pb-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Persamaan Linear (PLSV)</h3>
        </div>
        ...
        <p className="text-sm text-slate-500 mt-2 max-w-2xl leading-relaxed">
          Pahami konsep keseimbangan nilai di kedua ruas persamaan dan penggunaan operasi invers untuk mengisolasi variabel x.
        </p>
      </div>

      {/* Visualizations List */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {plsvVisualizations.map((item) => (
          <PlsvCard key={item.id} item={item} />
        ))}
      </div>
    </div>
  );
