    ...
    <div className="max-w-4xl mx-auto py-6 animate-fadeIn pb-24 md:pb-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Sorter Suku Sejenis</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8">
          {mode === 'explore' && (
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col relative overflow-hidden min-h-[500px]">
              
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-bold text-slate-700">Pilih suku, lalu masukkan ke keranjang!</h3>
                <button onClick={handleReset} className="p-2 text-slate-400 hover:text-rose-500 transition-colors" title="Reset">
                   <RotateCcw size={20} />
                </button>
              </div>

              {/* Scattered Terms */}
              <div className="flex flex-wrap gap-3 mb-8 min-h-[80px] p-4 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                {unassignedTerms.length === 0 ? (
                  <div className="w-full text-center text-emerald-500 font-bold flex flex-col items-center gap-2">
                  ...
                  unassignedTerms.map(term => (
                    <button
                      ...
                      className={`px-4 py-2 text-xl font-bold font-mono rounded-xl border-2 transition-all shadow-sm ${selectedTerm?.id === term.id ? 'bg-indigo-500 border-indigo-600 text-white scale-110 shadow-md ring-4 ring-indigo-200' : 'bg-white border-slate-200 text-slate-700 hover:border-indigo-300 hover:text-indigo-600'}`}
                    >
              ...
              {errorMsg && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-rose-600 text-white font-bold px-6 py-3 rounded-full shadow-lg z-20 animate-bounce">
                  {errorMsg}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4 mt-auto">
                {[
                  ...
                  <div 
                    ...
                    className={`flex flex-col min-h-[120px] rounded-2xl border-2 transition-all cursor-pointer ${selectedTerm ? `hover:bg-${bin.color}-50 border-${bin.color}-300 border-dashed` : 'border-slate-200 bg-white'}`}
                  >
                    <div className={`text-center py-2 text-sm font-bold text-${bin.color}-700 bg-${bin.color}-100/50 rounded-t-xl border-b border-${bin.color}-200/50`}>
                      {bin.label}
                    </div>
                    <div className="p-3 flex flex-wrap gap-2 content-start flex-1">
                      {bins[bin.id].map(term => (
                        <div key={term.id} className={`px-3 py-1 bg-${bin.color}-50 text-${bin.color}-800 border border-${bin.color}-200 rounded-lg font-mono font-bold text-sm shadow-sm`}>
                          <InlineMath math={term.math} />
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-6">
                    {['Ya', 'Tidak'].map((ans, idx) => (
                      <button 
                        ...
                        className={`px-8 py-4 rounded-2xl border-2 font-bold text-xl transition-all hover:scale-105 hover:shadow-md ${evalResult !== 'none' ? 'pointer-events-none' : ''} ${
                           evalResult === 'correct' && (
                              (quizStep === 0 && ans === 'Tidak') || 
                              (quizStep === 1 && ans === 'Ya') || 
                              (quizStep === 2 && ans === 'Ya')
        ...
        <div className={`lg:col-span-4 flex flex-col gap-4 ${mode === 'evaluate' ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="bg-amber-50 border border-amber-200 p-6 rounded-3xl shadow-sm">
            <h3 className="text-amber-800 font-bold mb-4 text-sm flex items-center gap-2">
               Syarat Suku Sejenis
            </h3>
            <ul className="text-sm text-amber-900 font-medium space-y-3 pl-4 list-disc marker:text-amber-500">
               <li>Memiliki <strong>variabel yang sama persis</strong>.</li>
               <li>Memiliki <strong>pangkat yang sama persis</strong>.</li>
            </ul>
            <div className="mt-4 pt-4 border-t border-amber-200/50 flex items-start gap-2">
