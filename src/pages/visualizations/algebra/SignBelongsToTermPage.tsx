    ...
    <div className="max-w-4xl mx-auto py-3 lg:py-6 animate-fadeIn pb-24 md:pb-6 px-3 sm:px-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 lg:mb-8 gap-3">
        <div>
          ...
          <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tight">Tanda Milik Suku</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-8">
        ...
        <div className="lg:col-span-8">
          {mode === 'explore' && (
            <div className="bg-white border border-slate-200 rounded-2xl lg:rounded-3xl p-3 lg:p-12 shadow-sm min-h-[300px] lg:min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden">
              
              <div className={`flex items-center text-2xl lg:text-5xl font-mono font-black mb-8 lg:mb-12 transition-all duration-700 ${isCut ? 'gap-3 lg:gap-6' : 'gap-0 bg-slate-50 border-2 border-slate-200 p-3 lg:p-4 rounded-xl'}`}>
                <div className={`transition-all duration-500 ${isCut ? 'bg-indigo-50 border-2 border-indigo-200 p-2 lg:p-4 rounded-xl shadow-sm text-indigo-700' : ''}`}>
                  <InlineMath math="4x" />
                </div>
                {!isCut && <div className="mx-1 lg:mx-2 text-slate-400"><InlineMath math="-" /></div>}
                <div className={`transition-all duration-500 flex ${isCut ? 'bg-rose-50 border-2 border-rose-200 p-2 lg:p-4 rounded-xl shadow-sm text-rose-700' : ''}`}>
                  {isCut && <span className="mr-1.5 lg:mr-2"><InlineMath math="-" /></span>}
                  <InlineMath math="2y" />
                </div>
                {!isCut && <div className="mx-1 lg:mx-2 text-slate-400"><InlineMath math="+" /></div>}
                <div className={`transition-all duration-500 flex ${isCut ? 'bg-emerald-50 border-2 border-emerald-200 p-2 lg:p-4 rounded-xl shadow-sm text-emerald-700' : ''}`}>
                  {isCut && <span className="mr-1.5 lg:mr-2"><InlineMath math="+" /></span>}
                  <InlineMath math="5" />
                </div>
              </div>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 lg:px-6 lg:py-3 rounded-xl font-bold transition-all border-2 text-xs lg:text-base ${isCut ? 'bg-slate-100 text-slate-600 border-slate-300 hover:bg-slate-200' : 'bg-indigo-500 text-white border-indigo-600 shadow-sm hover:bg-indigo-600'}`}
              >
                <Scissors size={16} />
                  ...
                  <div className="flex gap-4">
                    {['1', '-1', '0'].map((ans, idx) => (
                      <button 
                        ...
                        className={`w-16 h-12 sm:w-20 sm:h-16 rounded-xl border-2 font-bold text-sm sm:text-lg transition-all hover:scale-105 hover:shadow-md ${evalResult !== 'none' ? 'pointer-events-none' : ''} ${evalResult === 'correct' && ans === '-1' ? 'bg-emerald-100 border-emerald-400 text-emerald-700' : evalResult === 'wrong' && ans !== '-1'  ? 'bg-rose-50 border-rose-200 text-rose-400' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600'}`}
                      >
                        {ans}
                      </button>
                    ))}
                ...
                <div className="flex gap-4">
                  {['9', '-9', '0'].map((ans, idx) => (
                    <button 
                      ...
                      className={`w-16 h-12 sm:w-20 sm:h-16 rounded-xl border-2 font-bold text-sm sm:text-lg transition-all hover:scale-105 hover:shadow-md ${evalResult !== 'none' ? 'pointer-events-none' : ''} ${evalResult === 'correct' && ans === '-9' ? 'bg-emerald-100 border-emerald-400 text-emerald-700' : evalResult === 'wrong' && ans !== '-9'  ? 'bg-rose-50 border-rose-200 text-rose-400' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600'}`}
                    >
                      {ans}
                    </button>
                  ))}
        ...
        <div className={`lg:col-span-4 flex flex-col gap-3 lg:gap-4 ${mode === 'evaluate' ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="bg-emerald-50 border border-emerald-200 p-3 lg:p-6 rounded-2xl lg:rounded-3xl shadow-sm">
            <h3 className="text-emerald-800 font-bold mb-1.5 lg:mb-2 text-xs lg:text-sm flex items-center gap-2">
               Tanda di Depan = Milik Suku Tersebut
            </h3>
            <p className="text-xs lg:text-sm text-emerald-700 leading-relaxed mb-3 lg:mb-4">
            ...
            <div className="bg-white p-2.5 lg:p-4 rounded-xl border border-emerald-100 mb-3 lg:mb-4 text-center font-mono font-bold text-[10px] lg:text-xs text-slate-700">
               -2y berarti sukunya adalah negatif 2y
            </div>
            <div className="flex items-start gap-2 pt-2 border-t border-emerald-200/50">
              <Info className="text-emerald-500 shrink-0 mt-0.5" size={14} />
              <p className="text-[10px] lg:text-xs text-emerald-600/80">
