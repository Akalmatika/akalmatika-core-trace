      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-8">
        ...
        <div className="lg:col-span-8">
          ...
          <div className="w-full flex flex-col items-center gap-3 lg:gap-6 relative z-10 max-w-lg mt-2 lg:mt-4">
             ...
             <div className={`w-full h-12 lg:h-16 flex border-2 border-slate-300 relative bg-slate-50 overflow-hidden shadow-inner transition-all duration-[1500ms] ease-in-out transform origin-top rounded-lg ${step === 1 ? 'opacity-0 scale-y-0 h-0 border-0' : 'opacity-100 scale-y-100'}`}>
                {Array.from({ length: den }).map((_, idx) => (
                  <div 
                    key={idx}
          ...
          <div className={`mt-4 lg:mt-12 text-center text-slate-500 font-medium bg-white/80 px-3 py-1.5 lg:px-4 lg:py-2 rounded-xl border border-slate-100 shadow-sm transition-all duration-[1500ms] ease-in-out text-[10px] lg:text-sm ${step === 1 ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className={`lg:col-span-4 flex flex-col gap-3 lg:gap-4 ${mode === 'evaluate' ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <div className="bg-rose-50 border border-rose-100 p-3 lg:p-6 rounded-2xl lg:rounded-3xl mt-1 lg:mt-4">
