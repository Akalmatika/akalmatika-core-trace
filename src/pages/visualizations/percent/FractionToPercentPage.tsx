    ...
    <div className="max-w-4xl mx-auto py-6 animate-fadeIn pb-24 md:pb-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Konverter Pecahan ke Persen</h2>
        ...
        <div className="lg:col-span-8">
          {mode === 'explore' && (
            <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-12 shadow-sm min-h-[450px] flex flex-col items-center justify-center relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12 relative z-10 w-full mt-4 mb-8">
                {/* 1. Base Fraction */}
                <div className="flex flex-col items-center text-5xl font-black font-mono text-slate-400 transition-all">
                  <div>{currentFraction.n}</div>
                  <div className="w-full h-1.5 bg-slate-300 my-2 rounded-full"></div>
                  <div>{currentFraction.d}</div>
                </div>

                <div className={`transition-all duration-500 flex flex-col md:flex-row items-center gap-8 ${step >= 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
                  <ArrowRight className="text-slate-300 hidden md:block" size={32} />
                  
                  {/* 2. xMultiplier /100 */}
                  <div className="flex flex-col items-center relative">
                    <div className="absolute -top-8 text-xs font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded shadow-sm">x{multiplier}</div>
                    <div className="flex flex-col items-center text-5xl font-black font-mono text-emerald-500">
                      <div>{percentValue}</div>
                      <div className="w-full h-1.5 bg-emerald-300 my-2 rounded-full"></div>
                      <div>100</div>
                    </div>
                    <div className="absolute -bottom-8 text-xs font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded shadow-sm">x{multiplier}</div>
                  </div>
                </div>

                <div className={`transition-all duration-500 flex flex-col md:flex-row items-center gap-8 ${step === 2 ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
                  <ArrowRight className="text-slate-300 hidden md:block" size={32} />
                  
                  {/* 3. Percent! */}
                  <div className="flex items-center gap-1 text-6xl font-black font-mono text-indigo-600 bg-indigo-50 px-6 py-4 rounded-3xl border-4 border-indigo-100 shadow-inner">
                    {percentValue}<span className="text-4xl text-indigo-400">%</span>
                  </div>
                </div>
              </div>
              
              <div className="flex gap-4 relative z-10 mt-8 w-full max-w-sm">
                <button
                  onClick={() => setStep(1)}
                  disabled={step !== 0}
                  className="flex-1 py-3 rounded-xl font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-emerald-50 text-emerald-600 border-2 border-emerald-500 hover:bg-emerald-100 active:scale-95"
                >
                  Ubah ke /100
                </button>
                <button
                  onClick={() => setStep(2)}
                  disabled={step !== 1}
                  className="flex-1 py-3 rounded-xl font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-indigo-500 text-white shadow-sm hover:bg-indigo-600 active:scale-95"
                >
                  Jadikan %
                </button>
              </div>
                ...
                <div className="flex gap-6">
                  {['20%', '50%', '12%'].map((ans, idx) => (
                    <button 
                      ...
                      className={`w-24 h-16 rounded-xl border-2 font-bold text-xl transition-all hover:scale-105 hover:shadow-md ${evalResult !== 'none' ? 'pointer-events-none' : ''} ${evalResult === 'correct' && ans === '50%' ? 'bg-emerald-100 border-emerald-400 text-emerald-700' : evalResult === 'wrong' && ans !== '50%'  ? 'bg-rose-50 border-rose-200 text-rose-400' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600'}`}
                    >
                      {ans}
                    </button>
                  ))}
                ...
                <div className="flex gap-6">
                  {['34%', '75%', '43%'].map((ans, idx) => (
                    <button 
                      ...
                      className={`w-24 h-16 rounded-xl border-2 font-bold text-xl transition-all hover:scale-105 hover:shadow-md ${evalResult !== 'none' ? 'pointer-events-none' : ''} ${evalResult === 'correct' && ans === '75%' ? 'bg-emerald-100 border-emerald-400 text-emerald-700' : evalResult === 'wrong' && ans !== '75%'  ? 'bg-rose-50 border-rose-200 text-rose-400' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-400 hover:text-indigo-600'}`}
                    >
                      {ans}
                    </button>
                  ))}
